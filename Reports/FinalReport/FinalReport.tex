...
works by iterating over groups of points and attempting to fit a line through them. It then first checks if the points conform to the
predicted line by calculating the distance between the point and the predicted point (which is where the point should be if it
conforms to the line), ensuring it is less than $\delta$. If this condition is met, the algorithm then checks if the point conforms to
the line by calculating the distance between the point and the line, ensuring it is less than $\epsilon$. If both conditions are met,
the algorithm returns the line segment. If no line segment is found, the algorithm will return null. The algorithm will execute in \(O(n)\)
time, where n is the number of points. The algorithm's memory complexity is \(O(1)\), as it is bounded by the number of points.\\
%TODO: check algorithm complexity properly
\begin{algorithm}[H]
        ...
        \Function{seed\_segment\_growing}{$line(i, j)$, $N_p$, $P_{min}$, $L_{min}$, $\epsilon$}
            ...
            \While{euclidean\_distance($P_f$, $line(i, j)$) < $\epsilon$}
                \If{P_f > $N_p$}
                    \State \textbf{break}
                \ELSE
                    \State $line \gets \textit{fit\_line(P_b, P_f)}$
                \EndIf
            \EndWhile
        \EndFunction{}
    \end{algorithmic}
