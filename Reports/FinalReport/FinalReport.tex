...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Procedure{lidar\_scan}{$environment$, $agent\_position$, $num\_rays$}
            \State $data \gets []$
            \For{$i \gets 0$ to $num\_rays$}
                ...
                \For{$j \gets 0$ to $max\_distance$}
                    ...
                    \If{$environment.is\_object($new\_x$ , $new\_y$)$}
                        \State $sensor\_activation \gets $j / $max\_distance$
                        \State $data.append(($angle , sensor\_activation$))$
                        \State $break$
                    \EndIf
                \EndFor
            \EndFor
    ...
    \end{algorithmic}
\end{algorithm}
The procedure \textit{lidar\_scan} takes three parameters: the environment, the agent's position and the number of rays to cast. It
also has a hard-coded maximum distance, which is used to limit the sensor's range. The algorithm will execute in \(O(d * r)\) time,
where d is the maximum distance and r is the number of rays. The algorithm's memory complexity is \(O(r)\), as it is bounded by the
number of rays.\\
